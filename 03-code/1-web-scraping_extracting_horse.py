...
HorseBasicsInfo = []
all_horsebasics_html = os.listdir( 'HorsesBasics_data_html')

for horse_basics_file_name in all_horsebasics_html[ :]:
    target_file_name = 'HorsesBasics_data_html\\' + horse_basics_file_name
...
pd.DataFrame( HorseBasicsInfo).to_csv( 'HorseBasicsInformation.csv', encoding = 'utf-8_sig')
...
tmp[ 'LastRace_nDays'] = tmp[ 'RDate'] - tmp[ 'LastRace_Date']
tmp[ 'LastRace_nDays'] = pd.to_numeric( tmp[ 'LastRace_nDays'] ) / 24 / 60 / 60 / (10 ** 9)
tmp[ tmp[ 'LastRace_nDays'] < 0][ 'LastRace_nDays'] = 36
